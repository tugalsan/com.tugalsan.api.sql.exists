...
public class TS_SQLExists { 
    ...
    public boolean whereGroupAnd(TGS_CallableType1_Run<TS_SQLWhereGroups> groups) {
        executor.where = TS_SQLWhereUtils.where();
        executor.where.groupsAnd(groups);
        return executor.run();
    }

    public boolean whereGroupOr(TGS_CallableType1_Run<TS_SQLWhereGroups> groups) {
        executor.where = TS_SQLWhereUtils.where();
        executor.where.groupsOr(groups);
        return executor.run();
    }

    public boolean whereConditionAnd(TGS_CallableType1_Run<TS_SQLWhereConditions> conditions) {
        return whereGroupAnd(where -> where.conditionsAnd(conditions));
    }

    public boolean whereConditionOr(TGS_CallableType1_Run<TS_SQLWhereConditions> conditions) {
